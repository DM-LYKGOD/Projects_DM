...
class HeatPumpHplibController(Component):
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        if force_convergence:
            ...
            # --- Addition for odd day and temperature restriction ---
            
            sim_start = self.my_simulation_parameters.start_date
            seconds_since_start = timestep * self.my_simulation_parameters.seconds_per_timestep
            current_date = sim_start + datetime.timedelta(seconds=seconds_since_start)
            daily_avg_outside_temperature_in_celsius = stsv.get_input_value(self.daily_avg_outside_temperature_input_channel)
            # Only allow ON if today is an odd day and temp < 13.5°C
            if not ((current_date.day % 2 == 1) and (daily_avg_outside_temperature_in_celsius < 13.5)):
                # print(f"FORCING OFF at {current_date} (day {current_date.day}), Temp: {daily_avg_outside_temperature_in_celsius}")
                stsv.set_output_value(self.state_channel, 0)
                return
