...
FILE_PATH = r"C:\Users\Debapratim Mukherjee\Google Workspace\Master thesis 2025\01_Data\Greifensee_data_pomati_2019to2022_taxa (1).csv"
OUTPUT_DIR = r"C:\Users\Debapratim Mukherjee\Google Workspace\Master thesis 2025\04_Visualizations"
...
def fig_feature_importance_by_category(rf_importance_df):
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure4_feature_importance_by_category.png')
...
def fig_env_feature_importance_v2(rf_importance_df):
    ...
    ax.invert_yaxis()
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure5_env_feature_importance.png')
...
def fig_density_predictability(rf_df):
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure1_density_predictability.png')
    plt.savefig(outpath, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {outpath}")
...
def fig_r2_heatmap(hybrid2_df, xgb_df, rf_df):
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure2_r2_heatmap.png')
    plt.savefig(outpath, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {outpath}")
...
def fig_rf_r2_histogram(rf_df):
    ...
    ax.grid(False)
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure3_rf_r2_histogram.png')
    plt.savefig(outpath, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {outpath}")
...
def fig_ablation_heatmaps(hybrid2_df, xgb_df, rf_df, ablation_df):
    ...
    plt.tight_layout()
    outpath = os.path.join(OUTPUT_DIR, 'figure6_ablation_lag_rolling.png')
    plt.savefig(outpath, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {outpath}")
...
def main():
    ...
    for df_res, name in [(hybrid2_df, 'hybrid2'), (xgb_df, 'xgb'), (rf_df, 'rf_clean')]:
        if not df_res.empty:
            df_res.to_csv(os.path.join(OUTPUT_DIR, f'{name}_all_species_results.csv'), index=False)

    if not rf_importance_df.empty:
        rf_importance_df.to_csv(os.path.join(OUTPUT_DIR, 'rf_feature_importance_by_species.csv'), index=False)
